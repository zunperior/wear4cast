    ...
    return (
      <div className="container">
        ...
        <ul>
          ...
          <li><code>{'valid'}</code>{', '}<code>{'invalid'}</code>{', '}<code>{'pristine'}</code>{' and '}<code>{'dirty'}</code>{' flags are passed with each change'}
          </li>
          <li><em>{'Except'}</em>{' when you submit the form, in which case they are shown for all invalid fields.'}</li>
          <li>{'If you click the Initialize Form button, the form will be prepopupated with some values and the '}<code>{'pristine'}</code>{' and '}<code>{'dirty'}</code>{' flags will be based on those values.'}
          </li>
        </ul>
